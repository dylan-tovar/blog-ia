...
- **Presentación de la portada de texto:** toda la tarjeta (no solo el bloque superior) usa el color de la paleta como una superficie continua y muestra solo el texto de la portada y el título, sin extracto; la vista previa del diálogo usa el mismo componente (`ArticleCardView`). Las tarjetas con imagen o sin portada no cambian.
