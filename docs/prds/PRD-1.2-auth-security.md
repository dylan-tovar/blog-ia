...
3. ¿ya hay fila en profiles?                   -> redirect("/") sin tocarla
4. insert { id, display_name, username }       -> nunca upsert
5. error 23505                                 -> se vuelve a consultar el perfil:
                                                  existe   -> redirect("/")  (doble envío)
                                                  no existe -> "Ese nombre de usuario ya está en uso."
6. redirect("/")
```

- El perfil lo crea la Server Action con la sesión del propio usuario y bajo RLS (política "Users can insert their own profile"). **No hay trigger** en la base ni migración asociada ([ADR 0024](../adr/0024-perfil-en-onboarding.md)).
...
- **La puerta de onboarding.** En los `GET` de página (no `POST`, no `/api`) de un usuario con sesión hace un `select id` a `profiles`: sin fila redirige a `/onboarding` (salvo que ya esté ahí); con fila, `/onboarding` redirige a `/`. Si la consulta falla, deja pasar y registra el error. La decisión es la función pura `getGateRedirect`, con tests. Las redirecciones copian las cookies renovadas por `getUser()`. Los prefetch no se distinguen de una navegación (Next.js quita esos headers), así que también pagan la consulta.
...
- [ ] Registro válido: cuenta creada y redirección a `/onboarding`; al completarlo, fila en `profiles` y redirección a `/`.
- [ ] Un username repetido se rechaza en `/onboarding` con un mensaje amable; un doble envío no muestra error.
- [ ] Un usuario con sesión y sin perfil es llevado a `/onboarding` desde cualquier página de `GET`; con perfil, `/onboarding` lo lleva a `/`.
...
2. Registrar un usuario (termina en `/onboarding`), completar nombre y username y comprobar en Supabase (Table editor → `profiles`) que existe la fila con su `username` en minúsculas.
...
5. ¿Qué hace el proxy además de redirigir a `/login`? ¿Qué costo tiene la puerta de onboarding? ¿Por qué no protege `/profile`?
6. ¿Por qué el proxy "es UX, no seguridad"? ¿Dónde está la seguridad real?
7. ¿Qué pasa si `completeOnboarding` recibe un `23505`, y por qué se vuelve a consultar el perfil en vez de mirar el nombre de la restricción?
