...
Cada persona puede cambiar su **nombre para mostrar** y su **nombre de usuario** desde `/settings`. La pantalla es una lista de filas (Perfil, Email, Teléfono, Usuario) que abren un cajón de edición; solo Perfil y Usuario editan de verdad. `/profile` no es una pantalla propia: redirige al perfil público del usuario ([PRD-3.3](PRD-3.3-author-profile.md)).
...
| `updateProfile`, `getCurrentProfile`, `isUsernameAvailable`, `getPublicProfile` | Crear el perfil al registrarse: [PRD-1.2](PRD-1.2-auth-security.md) |
| `updateProfileSchema` y `usernameSchema` | Subida de avatar (no existe) |
| `/profile` (redirección) | Cambio real de email o teléfono (no existe) |
...
- `usernameSchema`: recorta, pasa a minúsculas y exige `^[a-z0-9_]{3,20}$`. Lo reutiliza `registerSchema` ([PRD-1.1](PRD-1.1-auth-forms.md)).
...
Usa `upsert` (y no solo `update`) para cubrir el caso en que el perfil no se creó al registrarse (ver [PRD-1.2](PRD-1.2-auth-security.md)): quien entra a `/settings` puede completarlo.

**3. `queries.ts`.**

- `isUsernameAvailable(username)`: cuenta perfiles con ese username; si hay error de consulta lanza una excepción.
...
- Cuatro filas en una tarjeta: Profile (nombre), Email, Phone, Handle (`@usuario`), cada una con un botón "Edit" que abre un `Drawer`. La sección "Publications" tiene "Mis artículos" y el enlace "Ver posts" (`/posts`). Abajo, "Cerrar sesión" (formulario que llama a `signOut`).
- `drawerSection` decide el contenido del cajón:
  ...
  - `phone`: texto informativo.
- **El teléfono muestra `+584123847815` cuando el usuario no tiene teléfono.** Es un valor fijo escrito en el código (un dato inventado de maqueta), no un dato real.
...
| Phone | No: el cajón solo muestra el número (que puede ser el inventado) |

## Decisiones y por qué

| Decisión | Alternativas | Consecuencia |
| :--- | :--- | :--- |
| **`upsert` en `updateProfile`** | Solo `update` | Cubre el perfil que no se creó al registrarse. Registrado en [PRD-1-auth.md](PRD-1-auth.md) |
| **El username se puede cambiar cuando se quiera** ([ADR 0007](../adr/0007-login-por-username-con-secret-key.md)) | Fijarlo al registrarse | El login usa el nuevo de inmediato. Costo: los `@usuario` compartidos dejan de servir. Motivo de permitirlo: no registrado † |
...
| **Filas de cuenta con cajón de edición** | Un formulario simple | Se parece a una app móvil. Las filas de Email y Teléfono quedaron sin funcionalidad real |
...
5. Pulsar "Edit" en Phone: ver el número (si el usuario no tiene teléfono, aparece el valor inventado).
...
| Tarea | Dificultad |
| :--- | :--- |
| Reemplazar el teléfono inventado `+584123847815` por un texto como "No registrado" (igual que el email) | B |
| Traducir al español los textos en inglés de `AccountSettings`: "Account", "Profile", "Email", "Phone", "Handle", "Edit", "Publications" | B |
