...
| Código | `src/features/auth/components/{LoginForm,RegisterForm,LoginDrawer}.tsx`, `src/features/profile/components/OnboardingForm.tsx`, `src/features/auth/schemas.ts`, `src/features/auth/utils.ts`, `src/app/(auth)/login/page.tsx`, `src/app/(auth)/register/page.tsx`, `src/app/(auth)/onboarding/page.tsx` |
...
| Dentro | Fuera |
| :--- | :--- |
| `LoginForm`, `RegisterForm`, `OnboardingForm` y sus páginas `/login`, `/register` y `/onboarding` | Crear el usuario y el perfil en la base, resolver username a email, cerrar sesión: [PRD-1.2](PRD-1.2-auth-security.md) |
...
**6. Páginas `(auth)/login/page.tsx`, `register/page.tsx` y `onboarding/page.tsx`.** Una `Card` de `max-w-sm` centrada con el formulario. El grupo `(auth)` **no tiene layout propio**: no hay cabecera ni navegación, solo el layout raíz.
...
6. Tests: `pnpm vitest run src/features/auth/schemas.test.ts` (cubre `registerSchema`, `loginSchema`, `isEmailIdentifier`, `resolveAuthRedirect`; `onboardingSchema` está en `src/features/profile/schemas.test.ts`).
