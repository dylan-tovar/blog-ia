...
import { NotificationsRealtimeProvider } from "@/features/notifications/realtime/NotificationsRealtimeProvider";
...
async function DesktopSidebarLoader() {
  ...
  return <DesktopSidebar viewer={viewer} />;
}
...
async function SignedInChrome({ fab }: { fab?: (viewer: Viewer) => ReactNode }) {
  ...
  return (
    <>
      {fab?.(viewer)}
      <BottomNav viewerUsername={viewer.username} />
    </>
  );
}
