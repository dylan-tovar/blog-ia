...
function LoginBenefits() {
  return (
    <>
      {/* Separador visual */}
      <div className="relative mx-1 sm:mx-4 my-5 flex items-center justify-center">
      ...
      <div className="mx-1 sm:mx-4 mb-2 flex flex-col gap-3 rounded-xl border border-border/60 bg-muted/20 p-3.5">
    ...
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        {trigger === null ? null : trigger ? (
          ...
          <DialogTrigger type="button" className={defaultTriggerClass}>
            Iniciar sesión
          </DialogTrigger>
        )}

        <DialogContent className="sm:max-w-md max-h-[90dvh] overflow-y-auto p-6 rounded-2xl border border-border/80 shadow-2xl">
          <DialogHeader className="text-center px-1 pt-1 pb-2">
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen} showSwipeHandle>
      {trigger === null ? null : trigger ? (
        ...
        <DrawerTrigger type="button" className={defaultTriggerClass}>
          Iniciar sesión
        </DrawerTrigger>
      )}
      ...
      <DrawerContent className="mx-2 sm:mx-auto sm:max-w-lg mb-2 pb-6 pt-1 min-h-[75vh] max-h-[92dvh] overflow-y-auto flex flex-col [--drawer-inset:0.5rem] data-[swipe-direction=down]:rounded-2xl data-[swipe-direction=down]:border border-border/80 shadow-2xl after:hidden">
        <DrawerHeader className="text-center px-4 pt-3 pb-2">
